...
INT APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, INT cmd_show) {
	...
	while (running) {
		...
		ImGui::SameLine();
		if (ImGui::Button("Attach & Scan Suite") || (autoRefresh && (GetTickCount() - lastRefreshTime > 1000))) {
		...
		ImGui::SameLine();
		ImGui::Checkbox("Auto-Refresh (1s)", &autoRefresh);

		ImGui::SameLine();
		if (ImGui::Button("Refresh Proc List")) {
			inspector.RefreshProcessList();
		}
		...
		if (ImGui::BeginTabBar("InspectorTabBar")) {
			...
			if (ImGui::BeginTabItem("Active Processes")) {
				if (ImGui::BeginTable("ProcTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, ImVec2(0, 420))) {
					ImGui::TableSetupColumn("PID", ImGuiTableColumnFlags_WidthFixed, 100.0f);
					ImGui::TableSetupColumn("Process Executable Name", ImGuiTableColumnFlags_WidthStretch);
					ImGui::TableHeadersRow();

					for (const auto& proc : inspector.activeProcesses) {
						ImGui::TableNextRow();
						ImGui::TableSetColumnIndex(0); ImGui::Text("%d", proc.pid);
						if (ImGui::IsItemClicked()) {
							sprintf_s(pidInput, "%d", proc.pid);
						}
						ImGui::TableSetColumnIndex(1); ImGui::Text("%ls", proc.name.c_str());
						if (ImGui::IsItemClicked()) {
							sprintf_s(pidInput, "%d", proc.pid);
						}
					}
